...
Name: 'RoR comment Manager'
Blender: 240
Group: 'RoR'
Tooltip: 'Manages comments in RoR vehicles'
"""
...
def button_event(evt): 
	...
	comments = mesh.getVertGroupNames()
	#to get selection must exit from editMode
	wasEditMode = Window.EditMode()
	Window.EditMode(0)
	#
	# Group ON/OFF button events
	#
	if evt <20:
		toggles[evt] = 1 - toggles[evt]
		gname = comments[evt]
		gverts = mesh.getVertsFromGroup(gname)
		print gname, gverts
		for v in mesh.verts:
			if v.index in gverts:
				#print " vert " + str(v.index) +" will be selected " + str(toggles[evt])
				v.sel = toggles[evt]
		if wasEditMode: Window.EditMode(1)
		mesh.update()
		Draw.Redraw(1)
		#scn.update(1)
		#Blender.Redraw()
	...
	if evt >= 20 and evt < 40:
		gname = comments[evt-20]
		gverts = mesh.getVertsFromGroup(gname)
		for v in mesh.verts:
			#print " vert " + str(v.index) +" will be selected " + str(toggles[evt])
			v.hide = not (v.index in gverts)
			v.sel = not v.hide
			if v.index in gverts:
				toggles[evt-20] = 1
			else: toggles[evt-20] = 0
...
def gui():
	...
	Draw.Text("Comment Manager", "small")
	...
	Draw.PushButton("Hide Sel", 102, 5, offset -10, 70,16 , "Hide selected groups")
	Draw.PushButton("Hide non Sel", 103, 75, offset -10, 85,16 , "Hide non selected groups")
	obj = Blender.Object.GetSelected()
	names = []
	if len(obj) > 0:
		names = obj[0].getData().getVertGroupNames()
		
	for i in range(len(names)):
		Draw.Toggle(str(i), i, 25, offset + 375-(18*i), 25, 16, toggles[i],"Turn this Group ON and OFF")
		Draw.PushButton("O", i+20, 10, offset + 379-(18*i), 10, 10, "Solo this Group")
		Draw.PushButton("", i+60, 173, offset + 376-(18*i), 10, 10, "Select Objects in this Layer")
		Draw.PushButton(names[i], i+40, 50, offset + 375 -(18*i), 120, 16, "Click to change Group name")
		Draw.ColorPicker(i , 300,  offset + 375 -(18*i), 75, 18, (0.3, 0.3, 0.45),  "",  colorpick)
