...
class lineOfSection(object):
	...
	def setSection(self, sectionName):
		...
		if parser.commands.has_key(sectionName):
			...
			if idx > -1:
				...
				raise Exception('%s is not a valid neither command nor section nor sectionfooter' % sectionName)
	...
	def getDefinition(self, column= -1):
		...
		if col is a valid column, it returns the Column definition instead
		"""
		if column == -1:
...
#		print "-- end dictionary --"
		...
		result = ""
		sep = ""
		if self.inlineComment == 'wheels' and self.section in ['nodes', 'beams']: return result
...
#					self.tree[actualsection].append({'data':argumenttree, 'originline':lineno, 'section':actualsection, 'obj':obj})
		...
		if len(self.errorCount) > 0:
			print 'total errors interpreting this file: %d\n\n%s' % (len(self.errorCount), '\n'.join(self.errorCount))
		#self.checkNodes()
...
#		p.parse(sys.argv[argno])
	p.parse('C:\\Documents and Settings\\Lupas\\Mis documentos\\Rigs of Rods\\vehicles\\mis_pruebas\\sexto.trailer')
	for l in p.lines:
		if l.isHeader: print "\n"
		print l.getTruckLine()
	print "finished !!!!"
	#p.printtree()
